...
const UNIT_TYPE_HINTS: Record<CargoUnitType, string> = {
  ...
  breakbulk:
    'Oversized or special cargo — crates, machinery, or pieces that do not fit a standard unit.',
};
...
export function normalizeCargoUnitType(
...
export function getInspectionDisplayTitle(inspection: CargoInspection): string {
  ...
  return (
    inspection.uldId ||
    `${getUnitTypeLabel(unitType)} · AWB ${inspection.awbNumber}`
  );
}
